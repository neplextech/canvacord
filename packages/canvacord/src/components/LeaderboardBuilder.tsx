...
import { chunkArrayInGroups, fixed } from "../helpers/utils";
...
export type LeaderboardVariants = (typeof LeaderboardVariants)[keyof typeof LeaderboardVariants];
...
export class LeaderboardBuilder extends Builder<LeaderboardProps> {
  ...
  public async renderDefaultVariant() {
    ...
    return (
      <div className="h-full w-full flex relative">
        {background && (
          <img src={background.toDataURL()} className="absolute top-0 left-0 h-full w-full" alt="background" />
        )}
        <div className="py-[30px] flex flex-col items-center w-full">
          ...
          {this.renderDefaultPlayers(
            await Promise.all(
              options.players.filter((f) => !winners.includes(f)).map((m) => this.renderDefaultPlayer(m)),
            ),
          )}
        </div>
      </div>
      ...
      <div className="flex items-center bg-white/15 rounded-xl p-2 px-3 justify-between">
        <div className="flex justify-between items-center">
          ...
          <div className="flex flex-col justify-center ml-3">
            ...
            {username && <div className="text-lg font-medium text-gray-300 flex">{username}</div>}
          </div>
        </div>
  ...
  public async renderHorizontalVariant() {
    ...
    const playerGroupChunks = chunkArrayInGroups(options.players, 5) as Array<typeof options.players>;
        ...
        <div className="flex justify-center w-full m-0 my-5">
          ...
          <div className="flex flex-col items-center justify-center">
            {options.header?.title && (
              <div className="text-white font-semibold text-2xl flex">{options.header.title}</div>
            )}
            {options.header?.subtitle && (
              <div className="text-gray-300 font-medium flex">{options.header.subtitle}</div>
            )}
          </div>
        </div>
